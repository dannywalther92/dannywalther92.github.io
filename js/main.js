...
// ── SCROLL REVEAL ─────────────────────────────────────────────────────────
// Funktioniert auch für Inhalte, die ERST NACH dem initialen Laden dynamisch
// ins DOM eingefügt werden (z.B. News-Kacheln/Partnerliste aus news.js/
// partners.js, gerendert über components.js). Ein MutationObserver erfasst
// solche neuen .reveal-Elemente automatisch und hängt sie an denselben
// IntersectionObserver an – kein manuelles Nachpatchen pro Render-Funktion
// mehr nötig.
...
const observer = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      ...
      const delay = Array.from(all).indexOf(entry.target) % 3 * 100;
...
// Nachträglich eingefügte .reveal-Elemente automatisch erfassen
